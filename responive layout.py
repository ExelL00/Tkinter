...
class App(tk.Tk):
    def __init__(self,start_size):
        ...
        size_notifer=SizeNotifer(self,{600: self.create_medium_layout,300: self.create_small_layout,1200: self.create_large_layout})
        self.mainloop()
...
class SizeNotifer:
    def __init__(self,window,size_dict):
        ...
        self.window.bind('<Configure>',self.check_size)# musi byc na window update bo tak nawet chlop nwm hehe pyhonxD
        self.window.update()
    ...
    def check_size(self,event):
        print(event.widget,self.window)
        if event.widget==self.window:
            window_width=event.width
            checked_size=None

            for min_size in self.size_dict:
                delta_size=window_width-min_size
